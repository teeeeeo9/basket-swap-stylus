...
pub fn mul_div(a: U256, b: U256, mut denominator: U256) -> Result<U256, UniswapV3MathError> {
    ...
    if denominator <= prod_1 {
        if denominator.is_zero() {
            return Err(UniswapV3MathError::DenominatorIsZero(DenominatorIsZero{}));
        }
        return Err(UniswapV3MathError::DenominatorIsLteProdOne(DenominatorIsLteProdOne{}));
    }
...
pub fn mul_div_rounding_up(
    ...
    if a.mul_mod(b, denominator).is_zero() {
        ...
        Err(UniswapV3MathError::ResultIsU256MAX(ResultIsU256MAX{}))
    } else {
        Ok(result + ONE)
    }
}
