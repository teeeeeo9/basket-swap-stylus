...
use crate::utils::*;
use crate::utils_2::{u128_to_uint256};
use crate::full_math::{mul_div_rounding_up, mul_div, mul_div_96};
...
fn to_uint160(x: U256) -> Result<U256, UniswapV3MathError> {
    if x > MAX_U160 {
        Err(UniswapV3MathError::SafeCastToU160Overflow(SafeCastToU160Overflow{}))
    } else {
        Ok(x)
    }
}
...
pub fn get_next_sqrt_price_from_amount_0_rounding_up(
    ...
    if add {
        ...
        if !(product / amount == sqrt_price_x96 && numerator_1 > product) {
            Err(UniswapV3MathError::ProductDivAmount(ProductDivAmount{}))
        } else {
            let denominator = numerator_1 - product;
...
pub fn get_next_sqrt_price_from_amount_1_rounding_down(
    ...
    if add {
        ...
        if sqrt_price_x96 > quotient {
            ...
            Err(UniswapV3MathError::SqrtPriceIsLteQuotient(SqrtPriceIsLteQuotient{}))
        }
    }
}
...
pub fn get_next_sqrt_price_from_input(
    ...
    if sqrt_price_x96.is_zero() {
        return Err(UniswapV3MathError::SqrtPriceIsZero(SqrtPriceIsZero{}));
    } else if liquidity == 0 {
        return Err(UniswapV3MathError::LiquidityIsZero(LiquidityIsZero{}));
    }
